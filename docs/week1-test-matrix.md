...
| W1-04 | Classic    | Existing draft update          | `lock=false`, slug currently plugin-generated                                   | Change title only, no explicit regenerate           | Slug remains unchanged (no forced replace)                            | `lock=false`, `last_generated` unchanged or refreshed-for-compare     |
